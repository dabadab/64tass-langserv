...
- **CPU target**: `DocumentIndex.cpu`, defaulting to `6502i` - the NMOS set with
  ...
  holding anything but the default. It no longer gates the diagnostics: they are
  judged against the target IN FORCE, declared or defaulted, because staying
  silent otherwise means saying nothing about most real sources (verified: the
  user's own file declares nothing). The cost is real and was accepted - a project
  on a wider CPU that declares nothing is reported against `6502i` until it says
  otherwise, which is what `FILE_CPU` in `examples.test.ts` supplies for the
  45GS02 example that names its target nowhere. `cpuExplicit` survives only to
  decide whether to pass a `--m...` flag to the real assembler. Only the FIRST `.cpu` in a
...
- **Directive scopes**: `.proc`, `.block`, `.macro`, `.function`, `.struct`, `.union`, `.namespace`.
  ...
  `jsr +` from outside must still reach it (all verified).
...
- **Multi-symbol lookup**: `colors.(red, green, blue)` looks several names up in
  one scope at once, and `multiLookupRegions` in `diagnostics.ts` maps each name
  inside the parentheses to that scope before resolving it - they were being
  looked for at the cursor and all reported undefined. A `.(` with NO scope in
  front of it builds a list of keys (`dict(.(red, green), ...)`), which names
  nothing, so that region is skipped instead.
...
- **Float literals**: a decimal exponent is `e` and a binary one `p`, and the
  ...
  stop at the point, so a fixed-point table read as values in a row.
...
- **Directive hover**: `directiveDocs.ts` is GENERATED by
  `tools/generate-directive-docs.mjs` from the manual page, and holds the manual's
  own syntax and description for all 132 directives. Quoted rather than
  rewritten: unlike the opcode and addressing tables, what a directive MEANS
  cannot be probed from the assembler, so the manual is the only source there is
  and a paraphrase would just add a worse one. Several directives share one entry
  (`.pend` and `.endproc`), which the generator handles by attaching a `<dd>` to
  every `<dt>` above it. `directiveHover` outranks `symbolHover` because the
  assembler does the same - a macro named `byte` does NOT take over `.byte`
  (verified) - but stays below `closerHover`, so a matched `.pend` still names its
  scope and only a stray one falls through to the description. A unit test asserts
  every `ALL_DIRECTIVES` entry has documentation, which is the reminder to re-run
  the generator when the tables gain a name. That check is also what caught `.org`
  sitting in `BUILTIN_DIRECTIVES_PATTERN`: 64tass has no such directive and reads
...
```bash
yarn test          # Run all tests (currently 1860 tests); compiles first
